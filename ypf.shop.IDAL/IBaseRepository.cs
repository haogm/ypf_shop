...
{
    /// <summary>
    /// 接口仓储基类（该接口规范了实现该接口的公共方法）
    /// </summary>
    /// <typeparam name="TG"></typeparam>
    public interface IBaseRepository<TG> where TG : class, new()
    {
        /// <summary>
        /// 删除
        /// </summary>
        ...
        /// <returns></returns>
        IQueryable<TG> LoadPages<T>(int pageIndex, int pageSize, out int totalcount, Func<TG, bool> where, Func<TG, T> orderBy, bool desc);
